...
module Atom
  ...
  class Entry
    ...
    def to_hash
      { :entry_id => id, :author => author, :title => title.decode_html.to_utf8, :content => content.decode_html, :created_at => published, :updated_at => updated }
    end
  end
end
...
Qt.app do
  window :main do
    ...
    has :item_list, :list_box do |il|
      il.current_item_changed { |c, p| select_item(c, p) }
    end
    
    has :item_text, :text_edit
